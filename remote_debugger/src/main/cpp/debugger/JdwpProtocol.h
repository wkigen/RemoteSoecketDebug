...
public:
	...
	bool waitRepNotify(JdwpReplyNotify* repEvent, long timeOut = 2000) {
		if (repEvent) {
...
//			}
	...
	bool waitJdwpResponse(JdwpPacket* jdwpPacket, JdwpRepObject* repObject, long timeOut = 2000) {
		bool bRet = false;
		if (!repObject) {
	...
	void workRoutine() {
		if(tryHandshake("127.0.0.1", 8000)){
			while (true) {
